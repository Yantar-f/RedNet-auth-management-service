...
public class AuthServiceImpl2 implements AuthService {
    ...
    public RegistrationCredentials signup(SignupRequestBody request) {
        ...
        Registration registration = registrationService.createRegistration(new RegistrationData(
                activationCode,
                tokenID,
                request.username(),
                request.email(),
    ...
    public Session signin(SigninRequestBody request) {
        ...
        return sessionService.createSession(account.getID(), account.getRoles().toArray(EnumRoles[]::new));
    }
